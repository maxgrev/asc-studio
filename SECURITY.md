...
- Multi-account Analytics keeps raw Apple issuer, app, report-request, and child-sync identifiers inside the local agent. V2 browser responses use opaque digests, and the legacy V1 lookup path cannot traverse portfolio child-run identifiers. The persisted Analytics roster contains nonsecret app metadata and per-credential membership only; private keys remain in macOS Keychain.
...
- Selected release content goes to OpenAI only after the user runs Translate & adapt. Requests include only selected description, What’s New, promotional text, and keywords; keywords are adapted for the target locale and search behavior. Support and marketing URLs are copied locally unchanged and are never sent. Review drafting sends only the authoritative review's rating, title, and body, followed by a separate verification request for the proposed reply.
