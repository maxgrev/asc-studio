...
export const AppleAdsWorkspace = ({ app, status, onUseInMetadata }: AppleAdsWorkspaceProps) => {
  ...
  const loadWorkspace = useCallback(async (quiet = false) => {
    ...
    setError(null);
    try {
      const [nextStatus, campaignResponse] = await Promise.all([
        api.appleAdsStatus(),
        api.appleAdsCampaigns(app.id),
      ]);
