...
- Current complete workflows cover TestFlight builds and group assignment; release versions, builds, localized What’s New, readiness, submission, and status; the canonical Store Listing editor and guarded screenshot sets; assisted What’s New translation; app-scoped written customer reviews, grounded reply drafting, and guarded public responses; portfolio-to-app App Analytics; multiple App Store Connect accounts; and Apple Ads research, reporting, and guarded campaign management.
- Store Listing is a separate top-level workspace and the only editor for version-localized description, promotional text, keywords, marketing and support URLs, and screenshots. Its copy drafts persist for the browser session with exact per-field intent; screenshot staging survives its Copy & search/Screenshots mode switch, requires explicit discard before navigation, and blocks departure while confirmation applies. Releases owns version and build selection, What’s New, readiness, submission, and status, surfaces pending storefront work, and routes remediation to the exact Store Listing version, locale, and field. Apple Ads routes selected keyword suggestions to that exact Keywords field.
...
- OpenAI writing assistance uses one local-agent BYOK configuration for What’s New translation and review replies. **Connections → Writing assistance** accepts a key and optional model once; the browser briefly holds and sends them to the authenticated loopback agent, never stores the key, and never receives it back.
