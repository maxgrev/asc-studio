...
- Current complete workflows cover TestFlight builds and group assignment; the unified Releases workbench for version, build, all six localized fields, screenshots, exact-diff review, readiness, submission, and status; assisted translation and keyword adaptation; app-scoped written customer reviews, grounded reply drafting, and guarded public responses; guarded purchasing-power-parity subscription pricing; portfolio-to-app App Analytics; multiple App Store Connect accounts; and Apple Ads research, reporting, and guarded campaign management.
- Subscriptions lists auto-renewable products and their Apple price schedules, then builds future-dated purchasing-power-parity plans from World Bank PPP and market-exchange-rate indicators normalized to a chosen base storefront. Careful, balanced, and stronger policies close 25%, 50%, or 75% of the measured local price-level gap. The configurable revenue floor spans 60–90% and defaults to 70%; recommendations never exceed Apple's comparable price, round the guarded factor upward to a five-point band, select an actual base-storefront Apple price point, and use its exact equalizations. Storefronts with missing evidence, a missing or currency-mismatched Apple equalization, or an existing future schedule stay unchanged. Exact territory diffs, source years, floor interventions, and preserved-subscriber raises remain visible through confirmation; current and scheduled prices are re-read before any write. Price creates are not retried; failed or indeterminate attempts are reconciled against Apple, and any schedules attributable to the attempt are removed. If exact restoration cannot be verified, the operation fails with a manual-review requirement.
- Releases is the single version-scoped operational workbench. Creating a version copies description, promotional text, keywords, support URL, and marketing URL from the selected previous version while leaving What’s New empty. Localized metadata drafts persist per app, version, and locale for the browser session with exact per-field intent, including migration of legacy Store Listing drafts. Screenshot staging survives the Localized content/Screenshots mode switch, requires explicit discard before departure, and blocks departure while an apply is running. Pending content or screenshots block readiness and submission. Readiness remediation and Apple Ads keyword handoff route to the exact Releases version, locale, and field; the keyword handoff opens **Localized content → Keywords**. Mobile keeps labeled readiness and submit actions in a persistent safe-area dock.
- Analytics opens on one portfolio spanning every connected App Store Connect account, independently of the operational account switcher, then preserves range, comparison, and selected metric while narrowing to one app. It exposes only additive metrics and ratios recomputed from additive totals, keeps per-source report provenance and freshness visible, and distinguishes zero, missing, partial, and privacy-withheld data.
...
- OpenAI writing assistance uses one local-agent BYOK configuration for Releases translation and keyword adaptation plus review replies. In **Translate & adapt**, users select fields and target locales: description, What’s New, and promotional text are translated; keywords are adapted under comma-list and 100-character rules; selected support and marketing URLs are copied locally unchanged and never sent to OpenAI. Only selected model-backed fields are sent, and only selected fields are applied atomically per locale. **Connections → Writing assistance** accepts a key and optional model once; the browser briefly holds and sends them to the authenticated loopback agent, never stores the key, and never receives it back.
...
- Direct IPA/PKG upload, remaining monetization work beyond subscription parity pricing, distribution, activity, additional submission controls, version-scoped review browsing, aggregate storefront ratings, review-response deletion, deeper analytics cohorts/reconciliation, scheduled analytics sync, and analytics AI explanations remain future work.
