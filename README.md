...
**Optimize search**, inside Releases → Localized content, combines the selected locale’s Name, Subtitle, and Keywords with a live text preview, character counts, word overlap checks, and up to four comparison variants saved in the browser session. Keyword cleanup removes repeated single words and redundant comma-list entries while preserving multiword phrases. It does not infer stemming, phrase indexing, organic rankings, or ranking improvements. Apple Ads research supplies app suggestions and storefront/category popularity for the last four complete weeks; an Apple Ads connection is required for research, while preview and draft checks work independently. Each result shows which draft fields contain its words and offers additions to Name, Subtitle, or Keywords when space permits.

Name and Subtitle come from Apple’s shared app-information record and affect all platforms. Keywords belong to the selected platform version. The optimizer selects the current editable app-information record for an editable release, requires the locale to exist in both resources, and fails closed on an ambiguous selection. Its review shows the exact changes and shared scope, rechecks both records before applying, patches only changed fields, and reads them back before reporting success. A failed or indeterminate save preserves the working draft and requires refreshing current values before reviewing another attempt. Exploration drafts and variants stay local until their separate review and confirmation; they are not pending release changes. Existing release keyword drafts are included when starting a fresh optimizer draft, and a successful save clears only the corresponding keyword edit while preserving other release edits.
