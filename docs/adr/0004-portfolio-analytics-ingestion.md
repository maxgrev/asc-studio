...
The product needs one reliable answer path: start with the whole portfolio across every connected App Store Connect account, identify which apps moved it, and narrow into one app without changing the date, comparison, or metric definition.
...
- A portfolio sync includes only apps with a known active report request. Apps that still need setup, and configured requests for which Apple has not generated files yet, are readiness states rather than failed downloads. A failed report-setup check does not invalidate a separately successful app connection; a current app route may retry the report call, while genuinely cached-only apps remain unroutable.
- Live portfolio discovery does not run alongside ingestion. While a sync is active, readers receive the stable catalog snapshot instead of issuing a second burst of Apple requests.
...
- Portfolio discovery groups saved credentials by Apple issuer, unions their proven app rosters, and routes each app through a credential that has demonstrated access. Public source, account, app, request, snapshot, and evidence identifiers are opaque digests; raw issuer IDs and Apple resource IDs remain server-side.
- The last successful nonsecret issuer/app roster is persisted so cached facts remain attributable after restart or a temporary Apple failure. An unproven or incomplete credential/source makes coverage explicit and partial; stale apps remain visible but cannot be used for mutations or sync routing.
...
The default GUI can truthfully aggregate the proven app roster from every connected organization and rank the apps behind a portfolio change. Switching the active operational credential does not alter that scope. Selecting an app reuses the same normalized evidence model for territory, source, page type, and version drill-downs instead of exposing Apple's report menu.
